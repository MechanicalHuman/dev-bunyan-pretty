...
const { printHead, colors } = require('../constants')
...
function formatHead (record) {
  ...
  head.loggerId = printHead ? record.name : ''
  ...
  head.color = color[record.level] || 'info'
  head.thingy = (process.platform === 'win32' || process.platform === 'win64') ? '>' : '❯'

  // TODO: Hanging indent on the header

  return formatter.render(template(head), colors)
}
