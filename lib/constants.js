...
Constants.STRICT = config.get('strict')
Constants.DEPTH = config.get('depth')
Constants.ARRAY_LENGTH = config.get('maxArrayLength')
Constants.MIN_LEVEL = config.get('level')

Constants.TIME_STAMPS = config.get('timeStamps')
Constants.TIME_STAMPS_FORMAT = config.get('stampFormat')
Constants.TIME_STAMPS_ZONE = config.get('timeZone')

Constants.USE_COLORS = config.get('colors')
...
Constants.set = (key, value) => {
  if (lo.isNil(value)) return
  lo.set(Constants, key, value)
}
