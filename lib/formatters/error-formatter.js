...
function render (record) {
  ...
  const stack = new Stack(err)

  let stackFrames = stack.stackFrames.filter(s => s.kind === 'Application')
  if (stackFrames.length === 0) stackFrames = stack.stackFrames

  stackFrames = prettyStack(stackFrames)

  if (stackFrames.length > 0) {
    output.push(
      chalk.red('  stack: ') + chalk.dim(`[${stackFrames.length - 1} Frames]`)
    )
...
function prettyStack (stack) {
  ...
  frames.forEach(function (frame) {
    ...
    if (frame.kind === 'Application') {
      var filepath = shortPath(frame.path)
