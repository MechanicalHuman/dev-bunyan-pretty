...
module.exports = function formatHead (record) {
  ...
  const name =
    record.app && record.app !== record.name
      ? Constants.PRETTY_PREFIX
        ? color(`${Constants.PRETTY_PREFIX}:${record.app}:${record.name}`)
        : color(`${record.app}:${record.name}`)
      : Constants.PRETTY_PREFIX
        ? color(`${Constants.PRETTY_PREFIX}:${record.name}`)
        : color(record.name)
...
function getavailableColumns (name, tag) {
  ...
  const available = Config.columns - (extras + headers)
  debug('available columns: %s', available)
