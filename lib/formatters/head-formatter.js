...
module.exports = function formatHead (record) {
  ...
  const name = record.appName
    ? color(`${record.appName}:${record.name}`)
    : color(record.name)
  const tag = chalk.white.dim(getTag(record))

  if (utils.isHttp(record)) {
    if (!record.msg) {
      return [name, httpFormatter(record), tag].join(Constants.SPACE_CHAR)
    }
    const httpMsg = httpFormatter(record) + ' - ' + record.msg
    return [name, httpMsg, tag].join(Constants.SPACE_CHAR)
  }

  record.msg = record.msg ? record.msg : chalk.dim('[no msg]')

  const extraHEader = Config.timeStamps ? 24 : 10
  const rest =
    stringLength([name, tag].join(Constants.SPACE_CHAR)) + extraHEader
