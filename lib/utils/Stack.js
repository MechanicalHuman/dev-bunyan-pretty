...
function Stack (error) {
  this.stackFrames = formatStack(getStack(error))
}

Stack.prototype.getCaller = function getCaller () {
  if (
    this.stackFrames.length === 0 ||
    this.stackFrames[0] instanceof StackFrame === false
  ) {
...
Stack.prototype.reduce = function simpleStack (n) {
  var stack = []

  // Try to select just the Application Errors;
  this.stackFrames.forEach(function (elem) {
    if (elem.kind === 'Application') {
      stack.push(elem)
    }
  })
  if (stack.length > n) {
...
StackFrame.prototype.toString = function FrametoString () {
  var context = this.kind
  if (this.kind === 'Application') {
    context = printf('%s:%s', this.file, this.line)
  }
  if (this.kind === 'External') {
    context = printf('%s:%s', this.path, this.line)
  }
  return printf('%s (%s)', context, this.fn)
}
...
function formatStack (stack) {
  return lodash.each(stack, function (frame, key, collection) {
    ...
    if (lodash.isEmpty(filepath) === false) {
      ...
      if (filepath.indexOf(process.cwd()) > -1) {
        // Is on the CWD, but it can be on the modules folder...
        if (filepath.indexOf('node_modules') > -1) {
          thisframe.kind = 'Library' // Is on the modules folder.
        } else {
          thisframe.kind = 'Application'
        }
      } else {
        // Best aproach is to consider that native code dont have a path ...
        if (filepath.indexOf('/') === 0) {
          // Asuming we are loading this from somewhere in the FS
          thisframe.kind = 'External'
        } else {
          thisframe.kind = 'NodeJS'
        }
      }
    } else {
      if (frame.isnative()) {
