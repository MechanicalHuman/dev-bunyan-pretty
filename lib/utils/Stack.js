...
Stack.prototype.reduce = function simpleStack (n = 3) {
  var stack = []
  let hasLib = 0
  ...
  this.stackFrames.forEach(function (elem) {
    ...
    if (elem.kind === 'Library' && hasLib < n) {
      stack.push(elem)
      hasLib = hasLib + 1
    }
  })

  if (stack.length < 1) return this.stackFrames
  return stack
}
...
StackFrame.prototype.toString = function FrametoString () {
  const context =
    this.kind === 'Application'
      ? printf('%s:%s', this.path, this.line)
      : this.kind
  ...
  return lodash.each(stack, function (frame, key, collection) {
    ...
    if (lodash.isEmpty(filepath) === false) {
      ...
      thisframe.kind =
        filepath.indexOf('/') !== 0
          ? 'NodeJS'
          : filepath.indexOf('node_modules') > -1 ? 'Library' : 'Application'
    } else {
      if (frame.isnative()) {
        thisframe.file = 'V8'
        thisframe.path = 'V8'
