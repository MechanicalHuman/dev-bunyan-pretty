...
exports.normalizeRecordFields = (record) => {
  ...
  if (!fp.isNil(record.message)) record.msg = record.message
  if (fp.isNil(record.msg)) record.msg = ''
  if (fp.isNil(record.name)) record.name = 'app'

  if (fp.has('err.validationErrors', record)) {
    record = fp.pipe(
      fp.set('validationErrors', fp.get('err.validationErrors', record)),
      fp.unset('err.validationErrors')
    )(record)
  }

  if (fp.has('err.aggregateErrors', record)) {
    record = fp.pipe(
      fp.set('aggregateErrors', fp.get('err.aggregateErrors', record)),
      fp.unset('err.aggregateErrors')
    )(record)
  }
